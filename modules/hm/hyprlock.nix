...

{
  programs.hyprlock = {
    ...
    settings = {
      source = [
        "$HOME/hydenix/modules/hm/assets/macchiato.conf"
    ...
    extraConfig = ''
      ...
      label {
        monitor =
        text = cmd[update:60000] bash -c 'BATTERY_ICON=$(/home/oxce5/hydenix/modules/hm/config/battery.sh icon); BATTERY_PERCENT=$(/home/oxce5/hydenix/modules/hm/config/battery.sh percentage); echo "$BATTERY_ICON $BATTERY_PERCENT"'
        color = $green
        font_size = 12
        font_family = $font
        position = 30, 45
