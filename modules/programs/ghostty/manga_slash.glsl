...
float waterFlow(vec2 p, vec2 start, vec2 end, float time, float seed) {
    ...
    float taper = smoothstep(0.1, 0.5, t) * (1.0 - smoothstep(0.7, 1, t));
    float mainFlow = taper * 1.0 / (abs(acrossPath - waveOffset) * mix(350.0, 200.0, taper) + 0.005);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        float lineIntensity = taper * 0.4 / (abs(acrossPath - offset - wavePhase) * mix(260.0, 150.0, taper) + 0.008);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (moveDistance > 0.01 && progress < 1.0) {
